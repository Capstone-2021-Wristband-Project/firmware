...
RecognizeCommands::RecognizeCommands(tflite::ErrorReporter* error_reporter,
                                     ...
                                     uint8_t detection_threshold, int32_t suppression_ms,
...
TfLiteStatus RecognizeCommands::ProcessLatestResults(const TfLiteTensor* latest_results,
    ...
    std::array<int8_t, kCategoryCount> weights;

    std::copy(latest_results->data.int8, latest_results->data.int8 + kCategoryCount,
              weights.begin());

    auto silence = weights[0];
    auto unknown = weights[1];
    auto yes = weights[2];
    auto no = weights[3];
    ...
    // const auto num_above_threshold = std::count_if(
    //     weights.cbegin(), weights.cend(), [max](int weight) { return weight > *max - 32; });
    ...
    // if (latest_results->data.int8[2] > 32 + latest_results->data.int8[1]) {
    //     printf("YES %d", latest_results->data.int8[2] - latest_results->data.int8[1]);
    // }

    // if (latest_results->data.int8[3] > 32 + latest_results->data.int8[1]) {
    ...
    //     { previous_top_label_ = current_top_label; previous_top_label_time_ = current_time_ms;
    //     *is_new_command = true;
    // } else {
    //     *is_new_command = false;
    // }
