...
constexpr int32_t history_samples_to_keep =
    ((kFeatureSliceDurationMs - kFeatureSliceStrideMs) * (kAudioSampleFrequency / 1000));
...
static void CaptureSamples(void* arg) {
    ...
    while (1) {
        ...
        i2s_read((i2s_port_t)0, (void*)i2s_read_buffer, i2s_bytes_to_read, &bytes_read, 10);
        if (bytes_read <= 0) {
            ...
            int bytes_written =
                rb_write(g_audio_capture_buffer, (uint8_t*)i2s_read_buffer, bytes_read, 10);
